...
class EmployeeTree:
    ...
    def _get_tree(self):
        ...
        for employee in upper_level_employees_list:
            self.current_tree.append(employee)
            manager = employee
    ...
    #ToDo: ограничение количества подчиненных. Нужно, чтобы оно не влияло на параметр
    # employee_tree
    def _add_subordinates_to_tree(self, manager):
        subordinates = list(Employee.objects.filter(manager=manager)[:50])
